...
class AdmissionController extends Controller
{
    ...
    */
    private function handleUploads(Request $request, $formNum): array
    {
        ...
        foreach ($map as $input => $column) {
            if ($request->hasFile($input)) {
                ...
                $filePath = $file->storeAs("uploads/{$formNum}", $fileName, 'public');
                $uploads[$column] = $filePath;  // Save the relative path
            }
        }
        ...
        $student_adm_list = $students->map(function ($data) {
            return [
                ...
                'overall_status' => (function () use ($data) {
                    ...
                    if ($data->is_payment && !$data->is_approved && !$data->is_reject) return 'Paid';
    ...
    //     return $response->getReasonPhrase() == 'OK' ? true : false;
    // }
